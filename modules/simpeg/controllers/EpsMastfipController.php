...
class EpsMastfipController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $dataProvider->query->andFilterWhere(['<', 'B_08', 3]);
    ...
    {
        $pg = EpsMastfip::find()->count();
        $my = SimpegEpsMastfip::find()->count();

        $tgl = date('Y-m-d H:i:s', $zid/1000);

        return $pg.' '.$my.' '.$tgl;

        // $mysql = SimpegEpsMastfip::find();
        // if($mysql->count() != 0){
        //     $no = 0;
        //     foreach($mysql->all() as $my){
        //         $pg = EpsMastfip::find()->where(['B_02'] => );
