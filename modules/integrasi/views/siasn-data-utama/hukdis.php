                ...
                'buttons' => [
                    ...
                    'upload' => function ($url, $model){
                        return Html::button('<span class="fas fa-file-upload"></span>',[
                            'value' => Url::to(['/upload-dok-rw', 'id' => $model['id'], 'nip' => 882]), 
                            'title' => 'Upload Dokumen', 'class' => 'showModalButton btn btn-link btn-sm', 
                        ]);                            
                    },
                    'file' => function ($url, $model){
                        $tahun = $model['skTanggal'];
                        $file = 'HUKDIS_'.$tahun.'_'.Yii::$app->request->get('id').'.pdf';
                        $dok = 'efi/'.$model['pnsOrang'].'/'.$file;
                        if(!file_exists($dok)){    
                            return Html::a('<span class="fas fa-file-alt"></span>',
                                Url::to(['/siasn-get-dok', 'id' => $file, 'nip' => Yii::$app->request->get('id').'.pdf']),
